...
while(len(file_list) != 0):
  ...
  while(DATA):
    ...
    percentage = "{0:.1f}".format(100 * (progress / num_of_packet))
    filled_length = 65 * progress // num_of_packet
    empty_length = 65 - filled_length
    bar = '=' * filled_length + '-' * empty_length + ' ' +percentage + "% " + "file ID " + str(rep_id)
    print(bar + '\r', end='')
